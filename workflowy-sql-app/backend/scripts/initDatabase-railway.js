...
async function initializeDatabase() {
  ...
  try {
    ...
    // Crear tabla consultas (ESQUEMA ORIGINAL SIMPLE)
    ...
    // Crear tabla etiquetas (ESQUEMA ORIGINAL SIMPLE)
    await connection.execute(`
    ...
    // Crear tabla de relación consulta_etiqueta (ESQUEMA ORIGINAL SIMPLE)
    await connection.execute(`
    ...
    // Crear tabla versiones_consulta (ESQUEMA ORIGINAL SIMPLE)
    await connection.execute(`
    ...
    if (existingCount === 0) {
      // Insertar consultas de ejemplo (DATOS SIMPLES)
      ...
      for (const consulta of consultasEjemplo) {
        await connection.execute(
          ...
          [consulta.titulo, consulta.descripcion, consulta.sql_codigo, consulta.autor]
        );
      }
      
      console.log(`✅ Insertadas ${consultasEjemplo.length} consultas de ejemplo`);
      
      // Insertar etiquetas de ejemplo (DATOS SIMPLES)
      ...
      // Crear versiones iniciales para las consultas (ESQUEMA SIMPLE)
      console.log('\n🔄 Creando versiones iniciales de consultas...');
      const [consultasCreadas] = await connection.execute('SELECT id, sql_codigo FROM consultas');
      
      for (const consulta of consultasCreadas) {
        await connection.execute(
          'INSERT INTO versiones_consulta (consulta_id, sql_codigo) VALUES (?, ?)',
          [consulta.id, consulta.sql_codigo]
        );
      }
